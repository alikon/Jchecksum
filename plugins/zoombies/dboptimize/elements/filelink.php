 ...
 * JFormField Filelink
 * @version 1.7.0
 * @copyright Copyright (C) 2011 Joomler!.net. All rights reserved.
 * @license GNU/GPL
 * @author Joomler!.net
...
//		require_once dirname(dirname(__FILE__)).'/helper.php';
		...
		if(is_array($values) && count($values) > 0){
			...
			$query->where('type = '. $db->Quote('plugin'). ' AND element = '. $db->Quote('dbbackup'). ' AND folder = '. $db->Quote('cron'));
			$db->setQuery($query);
			$params = new JRegistry($db->loadResult());
			
			$task = trim( $params->get('taskfile') );
			$path = JPATH_SITE. DS. 'plugins'. DS. 'system'. DS. 'jbackup'. DS. 'backup'.DS;
      $path = JPATH_SITE. DS. 'tmp'.DS;
			if(!empty($task)){
				$token = $this->getToken('filebackup', 'taskfile', $params);
				$base = '<span class="colleft"><a href="'. JURI::base(). 'index.php?'. $token. '=1&file=%s" title="%s" target="_blank">%s</a></span>';
				...
				if(!empty($removeToken)){
					...
					'<a class="btn" href="'. JURI::base(). 'index.php?'. $token. '=1&'. $removeToken. '=1'. $querycid. '&%s">'.
					'%s</a></span>';
					;
				}
			...
			$links .= '<span class="value red">'. JText::_('HERE_IS_NO_BACKUP_FILE') . '</span>';
		}
	...
	{
		if(is_null($params)){
			$plugin = JPluginHelper::getPlugin('cron', 'dbbackup');
  ...
  public static function getBackupFiles()
	{
		jimport('joomla.filesystem.folder');
	 	...
	 	$path = JPATH_SITE. DS. 'plugins'. DS. 'system'. DS. 'jbackup' . DS. 'backup';
