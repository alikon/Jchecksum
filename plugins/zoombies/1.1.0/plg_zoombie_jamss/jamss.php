...
/**
 * Zoombie JAMSS plugin
 * Embedd a Antimalware scan script on Joomla! 
 * 
 * @author: Alikon
 * @version: 1.1.0
...
class plgZoombieJamss extends JPlugin {
    ...
    function plgZoombieJamss(&$subject, $params) {
    ...
    function goAliveJamss($time) {
        ...
        $this->jamssPatterns = array(
            ...
            array('(eval|gzuncompress|gzinflate|base64_decode|str_rot13|strrev)[\s/\*\#]*\([\s/\*\#]*(eval|gzuncompress|gzinflate|base64_decode|str_rot13|strrev)',
                'PHP: double GZINFLATE||GZUNCOMPRESS||B64||ROT13||EVAL||STRREV',
                '17',
            ...
            array('<\s*iframe',
                'IFRAME element',
                '18',
                'Found IFRAME element in code, please check if it\'s a valid code.'),
            ...
            array('is_writable[\s/\*\#]*\([\s/\*\#]*getcwd',
                ...
                '20',
                'Could be harmless, but used in some malware'),
        );
        ...
        $this->lang->load('plg_zoombie_jamss');
        ...
        JLog::add('Start task #' . $this->runned . ' ZoombieJamss');
        /*
          if (php_sapi_name() == 'cli' && empty($_SERVER['REMOTE_ADDR'])) {
          JLog::add('job from CLI: ZoombieJamss.');
          } else {
          JLog::add('job from WEB: ZoombieJamss.');
          }
         */
        $this->dbo = JFactory::getDBO();
        $this->_scan($time);
        $task_time = round(microtime(true) - $this->task_i_time, 3);
        JLog::add('End task: ZoombieJamss in ' . $task_time);
