...
class plgCronUserDetector extends JPlugin {
    ...
    var $testo = null;
     ...
     function plgCronUserDetector( &$subject, $params )
	{
		parent::__construct( $subject, $params );

	
	}
    function doCronuserdetector($time) {
        ...
        if (JPluginHelper::isEnabled('alikonweb', 'detector')) {
            // Add a start message.
            JLog::add('Start job: CronUserDetector.');
              if(php_sapi_name() == 'cli' && empty($_SERVER['REMOTE_ADDR'])) {
                JLog::add('job from CLI: CronUserDetector.');
            } else {
               JLog::add('job from WEB: CronUserDetector.');
            }    
             $this->dbo = JFactory::getDBO();
             $this->_detectu($time);
            JLog::add('End job: CronUserDetector.');
            return 4;
        } else {
            return 4;
        }
...
//JLog::add('query:'.$query);        
        ...
        // Startup reporting.
//		echo '000';
        JLog::add(JText::sprintf('DETECTOR_CRON_USER_ITEMS', count($rows)));
