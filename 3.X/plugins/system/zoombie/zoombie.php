    ...
    function doSatanJobs() {
        ...
        foreach ($plugins as $plugin) {
            ...
            if ($diff > $interval) {
                ...
                $sendmail = $params->get('sendmail', false);
                ...
                if (is_array($results) && ($results[0] == 4)) {
                    ...
                    if ($sendmail) {
                        $this->sendNotice($plugin->element);
                    }
                } else {
...
//
    ...
    protected function sendNotice($zoombie) {
        ...
        $toemail = $fromemail;
        $subject = $zoombie . ' : ' . $config->get('config.sitename');
        $body = "\n\n * " . $zoombie . " runned at " . date('d.m.Y, H:i:s', $now) . "\n";
        $body .= "\n\n * " . $zoombie . " by  http://www.alikonweb.it \n";
        $mailer = & JFactory::getMailer();
        $mailer->setSender(array($fromemail, 'Zoombie System'));
